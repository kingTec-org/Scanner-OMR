...
<div class="page-content">
            <div class="wrap">
              <h4 id="success" style="text-align:center;"></h4>
                    <form class="form-horizontal" role="form" action="<?php echo base_url('student/add'); ?>" method="post" id="student_add">
                        <input type="hidden" name="ID" value="<?php echo @$View[0]['ID'];?>">

                          <div class="form-group">
                              <label  class="col-sm-2 control-label">Batch : </label>
                              <div class="col-sm-9">
                                <select type="text" class="chosen-select form-control" placeholder="Select Batch" name="batch_ID" value="<?php echo @$View[0]['batch_ID']; ?>" required>
                                  <option selected disabled>Select Batch</option>
                                  <?php if($batch != NULL) {
                                    foreach($batch as $key => $value) { ?>
                                    <option value="<?php echo $value['ID']; ?>" <?php echo ($value['ID'] == @$View[0]['batch_ID']) ? 'selected' : '';?>><?php echo $value['title']; ?></option>
                                  <?php } } else { ?>
                          ...
                          <div class="form-group">
                              ...
                              <div class="col-sm-9">
                                <input type="text" class="form-control" placeholder="Name" name="name" value="<?php echo @$View[0]['name']; ?>" required>
                              </div>
                                <span id="name"></span>
                          </div>
                          ...
                          <div class="form-group">
                              ...
                              <div class="col-sm-9">
                                <input type="text" class="form-control" placeholder="Roll Number" name="roll_no" value="<?php echo @$View[0]['roll_no']; ?>" required>
                              </div>
                              <span id="name"></span>
                          </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $("#student_add").postAjaxData(function(result){
      if(result === 1)
      {
