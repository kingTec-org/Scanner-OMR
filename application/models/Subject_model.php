<?php
	class Subject_model extends CI_Model
	{
		...
		public function add_or_edit()
		{
			...
			if ($this->form_validation->run('subject') !== FALSE)
			{
				if(empty($post['ID']))
				{
					...
					$this->db->trans_start();
					$result = $this->db->insert('subject',$post);
				...
				else
				{
					...
					$this->db->where(array('ID'=>$post['ID']));
			 		$result = $this->db->update('subject',$post);
					$this->db->trans_complete();
					if ($this->db->trans_status() === FALSE)
					{
					    $result = FALSE;
			...
			if($ajax == NULL)
			{
				if ($org->num_rows() > 0) 
				{
					...
					foreach ($data['data'] as $key => $value) {
						$data['data'][$key]['Actions'] = '<a class="btn btn-sm btn-primary" href="'.base_url('subject/add/'.$value['ID']).'"><i class="zmdi zmdi-edit"></i> Edit</a> <button class="btn btn-sm btn-danger" onClick="deletef(\''.$value['ID'].'\',\''.base_url('subject/delete').'\')"><i class="zmdi zmdi-delete"></i> Delete</button>';
					}
				}
				else
				{
		...
		public function delete($id = NULL)
		{
			...
			$this->db->trans_start();
			$org = $this->db->get_where('subject',array('ID'=>$id));
			...
			{
				$deleted_data = array('user'=>$this->session->userdata('ID'),'deleted_on'=>date('Y-m-d h:i:s'),'table'=>'subject','data'=>$rec);
